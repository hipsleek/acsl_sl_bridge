...
void search_replace(int array[], size_t length, int old, int new) {
  ...
  loop invariant \forall size_t j; (\at(i, LoopEntry) <= j && j < i && \at(array[j], LoopEntry) == old) ==> (\at(array[j], LoopEntry) == new);
  loop invariant \forall size_t j; (\at(i, LoopEntry) <= j && j < i && \at(array[j], LoopEntry) != old) ==> (\at(array[j], LoopEntry) == \at(array[j], LoopEntry));
