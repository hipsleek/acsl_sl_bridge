...
(* Recognize an "upper bound" predicate over a Post variable:
   - x <  K
   - x <= K
*)
...
(* Choose the max-K upper bound. If same K appears multiple times,
   prefer <= over < (weaker).
*)
...
let loop_contract_of_spec (s : C.spec) : A.loop_contract option =
  ...
  | [] -> None
  | bs ->
      (* looping branches = those with a real variant (Term[...]) *)
      let looping =
        bs
      ...
      | [b] ->
          (* Keep old behavior: faithful invariant from that single loop clause *)
      ...
      | looping_bs ->
          (* Multi-branch loop: summarize looping-branch guards by taking the max upper-bound. *)
          let upper_bounds =
            looping_bs
          ...
          let inv_core_preds =
            ...
            | None ->
                (* If no recognizable bound, fall back to first looping branch's assumes *)
